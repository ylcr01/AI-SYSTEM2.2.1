...
- **仓库写任务**：编辑前进入专属 Worktree 并运行 `node "$env:AI_RD_OS_ROOT\40-脚本\task.mjs" 准备 --cwd "<Worktree>" --intent "<目标>" --acceptance "<验收>" --scope "<Scope>" --integration-target "<目标分支>"`。Codex 优先 managed；不可用立即用 `git worktree add --detach <路径> <起点>`，不得等待、报告占用或降级 Local。Local 仅只读或串行集成。保存 `taskId`，最小修改、提交、`交付`；`ready_to_integrate` 后立即`集成`。隔离重验通过才快进清理，否则目标不变并报告。`needs_rework` 用`保存`/`恢复`。仅 `waiting_acceptance` 且门禁有效时可称“本轮已交付”，不得伪造 `accepted`。
