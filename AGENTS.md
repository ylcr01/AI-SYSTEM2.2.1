...
- **只读工程分析与仓库上下文**：只读分析或修改仓库前，先运行 `node "$env:AI_RD_OS_ROOT\40-脚本\build-context.mjs" --cwd "<项目路径>" --intent "<目标>"`，只读取轻量结果与目标代码、直接测试；仅身份、路由或依赖诊断追加 `--full`。
- **轻量直达**：`continuity=ephemeral` 的 Quick/普通 Standard 局部任务，输出简短目标卡后实施最小 Diff，不创建 Task；只跑定点检查并报告失败、跳过和未验证项，不声称 Evidence、`waiting_acceptance` 或用户验收。
- **仓库写任务（正式 Task）**：`tracked|handoff-required`、Controlled、Structural、规格/Decision、外部写入、跨仓或 Worktree 任务，编辑前运行 `task.mjs 准备`。主工作区仅一个写 Task；并行任务独占 Worktree并声明 `--integration-target`。Worktree 先提交再集成；暂停用 `保存`，继续用 `恢复`。仅有效 `waiting_acceptance` 可称“本轮已交付”，不得伪造 `accepted`。
