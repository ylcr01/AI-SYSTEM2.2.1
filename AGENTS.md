...
- **仓库写任务**：编辑前运行 `node "$env:AI_RD_OS_ROOT\40-脚本\task.mjs" 准备 --cwd "<项目路径>" --intent "<目标>" --acceptance "<验收>" --scope "<授权路径>"`。同一项目只有一个写任务时可使用 Local/主工作区；存在并行写任务时，每个任务必须在准备前进入独立 Worktree，Codex 桌面端优先使用任务专属 managed Worktree，其他宿主使用 detached worktree，Local/目标分支工作区只保留一个写任务或执行串行集成。linked/detached worktree 还必须传入 `--integration-target "<目标分支>"`。保存 `taskId`，读取回执中的 Scope 与 `filesToRead`，实施最小 ChangeSet；worktree 任务先提交再运行 `交付`，其状态进入 `ready_to_integrate` 后，由单一集成者将 `resultCommit` 集成到目标分支并运行 `集成 --task-id "<taskId>" --cwd "<目标工作区>"`。`needs_rework` 暂不继续时应运行 `保存` 显式释放工作树，`恢复` 时重新竞争原工作树写权限。只有状态为 `waiting_acceptance` 才能说明已完成工程交付，模型不得替用户验收。
