...
status: active
lastVerifiedTaskId: task-20260901084223958-1049eb3e
...
- 负责：按显式 operation 将只读、普通仓库写入、正式 Task 和外部写入分路；选择实际可读的 Contract/Canonical；保证模型直达、可选终检、Evidence、Browser 执行、Worktree 占用、诊断、对话收口和完成轮次指标不失真；安全迁移旧非终态记录。
...
| 精确 Scope 并集 | 正式 Task 或用户明确限定文件时，通过重复 `--scope` 声明的文件或目录集合；每个 ChangeSet 文件必须位于至少一个 Scope 内 |
...
| 轻量直达 | `continuity=ephemeral` 的普通任务；在干净、可用且无已知并发写入的当前工作区由模型实施最小充分 Diff 和相称检查，默认不建立正式 Task、Evidence、结果记录或本地提交 |
| 直达终检 | 用户明确要求提交或记录轻量结果时的可选无状态复核；原样重验 Baseline HEAD、branch/detached、`gitRoot`、`gitCommonDir`、占用、显式 Scope（若有）和 Manifest 差异，不生成 Task 或 Evidence |
...
| BR-AIRD-ROUTE-001 | Context/CLI 显式接受 `operation=read|write|external-write`。`read` 始终只读且 `ephemeral`；`external-write` 始终为 Controlled/tracked 正式路由；普通 `write` 在干净、可用且无已知并发写入的当前工作区由模型直接实施并展示真实 Git Diff。意图关键词、目录名、文件类型、计划路径和模型自行推测的 Scope 只作为事实 Context，不自动创建正式 Task、Worktree、Evidence、结果记录或提交。正式 Task 仅由显式跟踪、交接、并行隔离或外部写入选择；用户明确要求提交或记录时才执行可选直达终检 | 只读误进写路由、普通修改承担完整控制面成本、规则替代模型理解，或并发/用户改动被覆盖 | 所有正式 `准备` 命令仍创建 tracked Task；轻量账本保留为显式记录能力，不是默认路径或第二套 Task 状态机；`package.json` 真实差异只决定相称检查 |
...
| BR-AIRD-ALIGN-001 | 只有调用方显式选择正式 Task 后，Structural 或 external-write Controlled 才要求 confirmed/delegated Goal Card；普通任务由模型结合目标、代码和调用方澄清真实歧义。Intent、Acceptance、Scope 名称和路径不得自动升级控制模式或建立正式闭环 | 形式化关键词替代模型理解，或真正需要持续跟踪的结构/外部任务缺少目标对齐 | 普通修改保持直达；显式正式任务仍保留 Goal Card 与重新对齐能力 |
...
| BR-AIRD-COMMIT-001 | 正式 Task 继续形成只包含其授权 Scope 的单一本地提交。普通任务默认保留可见 Git Diff，不自动提交或记录；只有用户明确要求提交或结果记录时，轻量结果才回传终检的 baseline HEAD/`gitRoot`/`gitCommonDir` 与语义变更指纹。任何路径都不得自动 Push，外部写入仍须单独授权 | 默认提交隐藏业务 Diff、混入用户改动或误触远程写入，或显式记录的回执与实际提交不一致 | 验证失败、跳过、身份/指纹不符、显式 Scope 越界或工作树不干净时不得记录成功结果 |
| BR-AIRD-WORKSPACE-001 | 写入预检返回事实路由：当前工作区只有在 Git 干净、身份明确、无活动写 Task 且无已知并发写入时才可 `local-direct`；脏、占用、并发、detached 主区或状态不确定时使用新 Worktree。预检为可直达环境返回含 HEAD、branch/detached、`gitRoot`、`gitCommonDir` 的 `directBaseline`；用户显式要求提交或记录时，终检原样复核这些事实、占用、显式 Scope（若有）和 Manifest 差异。正式 Task 始终独占 Worktree，创建和非写态重入写态时原子复核 | 普通任务固定承担 Worktree 成本，同提交/分支的其他 clone 被误认为同一直达上下文，或并发/用户已有改动被覆盖 | 预检/终检不写持久化状态；正式 `准备` 在 Local 仍返回 `WORKTREE_REQUIRED` |
| BR-AIRD-SCOPE-001 | 正式 Task 或用户明确限定文件时，重复 `--scope` 形成精确授权并集，逗号拼接和 glob 仍拒绝；ChangeSet 只要落在任一 Scope 内即合法，全部 Scope 外的文件必须失败关闭。普通任务未收到文件边界时不预造 Scope，完成范围由实际 ChangeSet 给出 | 形式 Scope 限制模型探索和最小充分实现，或明确文件授权被越界 | 每个显式 Scope 独立执行 Git Root、真实路径和符号链接边界检查；无显式 Scope 时只报告实际文件 |
