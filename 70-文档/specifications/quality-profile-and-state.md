...
| BR-AIRD-METRICS-001 | 首轮显式验收摘要必须同时报告 decided、unknown、rate 和 coverage；对话摘要必须分开报告 tracked、implicit closures、single-turn 和各类 follow-up。返工只计同一 Task 内显式用户退回，对话缺陷退回不得改写旧验收指标；旧记录缺少对话事实时保持 unknown | 小量已决定样本被表达成整体稳定结果，或自然换话题被伪造成显式验收/首次修复成功 | 小样本和无可比基线仍需保留警告；系统不生成 `firstPassResolved` |
