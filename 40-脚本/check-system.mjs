...
for (const relative of [
  ...
  '70-文档/decisions/DEC-INTEGRATION-FRESHNESS-001.md', '70-文档/decisions/DEC-AUTOMATIC-SERIAL-INTEGRATION-001.md', '80-运行记录/README.md'
...
if (!/TRANSITIONS/u.test(state) || !/withFileLock/u.test(state) || !/withIntegrationLock/u.test(state) || !/CURRENT_SCHEMA = 9/u.test(state) || !/outcomeMetrics/u.test(state) || !/ready_to_integrate/u.test(state) || !/待验收/u.test(state)) errors.push('State Manager 缺少 V9 状态分层、结果指标、集成转换或并发锁');
...
for (const marker of ['继续验证','重验集成','--allow-primary-write','--allow-risk-integration','--confirm-only']) if (!taskCli.includes(marker)) errors.push(`task.mjs: 缺少恢复或集成命令 ${marker}`);
