...
function instructionText(fallback) {
  ...
  return `# Personal AI R&D Operating System\n\nResolve \`AI_RD_OS_ROOT\` (fallback: \`${fallbackPath}\`); read \`AGENTS.md\`. Preflight before repository writes. An ephemeral Quick/ordinary Standard uses a clean, available Local checkout only if no known writer: minimal diff, one targeted check, then a scoped local commit and lightweight outcome record. Dirty, occupied, concurrent, or uncertain state routes to managed or deterministic detached fallback. Formal, Controlled, Structural, spec, external, or cross-repo writes use a task-dedicated Worktree plus Task/integration gates and a local result commit. Never bypass Scope/Evidence gates, claim delivery before \`waiting_acceptance\`, or perform external writes without explicit authorization. Never Push by default. Explicit acceptance is optional; with an exact prior \`continuation\`, record every distinct related follow-up and never scan for a pending Task. If unreadable, report degraded state; do only repo-grounded low-risk work.\n\nBrowser hard limits remain global: at most 4 smoke flows, 15 seconds each, 2-minute batch, hard 3-minute outer timeout. Stop on the first failure or timeout; no auto-retry; event waits at most 1 second. Terminate after 30 seconds without output and report. Over 4 flows/2 minutes or a full regression requires explicit user authorization. Report failures/skips/terminations/circuit breaks; enforce in runner. Other checks never substitute for browser verification.\n`;
}
