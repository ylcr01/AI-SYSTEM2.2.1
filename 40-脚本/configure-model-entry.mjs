...
function instructionText(fallback) {
  ...
  return `# Personal AI R&D Operating System\n\nResolve \`AI_RD_OS_ROOT\` (fallback: \`${fallbackPath}\`); read \`AGENTS.md\`. Operation=read/write/external-write; read stays read-only. Before repository writes, inspect Git cleanliness, branch identity and known writers, then load only relevant project facts. In a clean, available current checkout, the model owns semantic understanding, exploration, implementation, validation choice and minimality: edit directly, keep the business Diff visible, and do not create Task, Scope, Worktree, Evidence or a commit merely because intent keywords or paths look important. Dirty, occupied, concurrent or uncertain state uses an isolated Worktree to protect existing changes. A formal Task is an explicit capability for sustained tracking, handoff, parallel isolation or authorized external writes, not an automatic text classifier outcome. An explicit user file boundary remains enforceable; otherwise report the actual ChangeSet. Keep machine exchange artifacts outside the project and user-visible change review. Never Push or perform external/destructive actions without explicit authorization, and report verification facts truthfully.\n\nBrowser hard limits remain global: at most 4 smoke flows; 15s each; 2-minute batch; hard 3-minute outer timeout. Stop on the first failure or timeout; no retry/fixed wait over 1 second. Runner lacks 30-second heartbeat/no-output termination; report \`unimplemented\`. Over 4 flows/2 minutes or a full regression requires explicit user authorization. Report failed/skipped/terminated/circuit-broken checks. Other checks never substitute for browser verification.\n`;
}
